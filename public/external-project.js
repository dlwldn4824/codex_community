...
showSolution = function(data) {
  ...
  const showReverify = result => {
    ...
    solutionPage.innerHTML = `<p class="eyebrow">반영 후 재검증</p><h2>${passed ? "수정 사본이<br/>검증을 통과했습니다." : "수정 후에도<br/>검토 항목이 남았습니다."}</h2><p class="repair-intro">${escape(result.note)}</p><section class="repair-recheck"><span>01 · Codex 수정안 적용</span><h3>${result.applied.length}개 실제 적용 · ${result.proposed?.length || 0}개 제안</h3><p>원격 GitHub 저장소는 바꾸지 않았습니다. 새 로컬 복제본에서 실제로 적용된 파일만 아래에 표시합니다.</p></section><section class="repair-recheck"><span>02 · 빌드·테스트</span><h3>${result.verification?.buildAndTest === "PASS" ? "PASS" : "격리 실행 환경 필요"}</h3><p>${escape(result.verification?.buildAndTestReason || "빌드와 테스트 결과를 확인했습니다.")}</p></section><section class="repair-recheck"><span>03 · Semgrep 재실행</span><h3>${result.before.findings}건 → ${result.after.findings}건 · ${passed ? "PASS" : "남은 검토"}</h3><p>${passed ? "동일 규칙으로 다시 분석한 결과, 남은 항목이 없어 PASS입니다." : "실제로 적용된 수정이 없거나 일부 항목만 수정되어, 아래 항목은 프로젝트 맥락을 확인한 뒤 별도 수정안이 필요합니다."}</p></section><section class="recheck-list"><p class="eyebrow">실제 적용된 수정</p>${applied}</section><section class="recheck-list"><p class="eyebrow">남은 검토 항목</p>${remaining}</section><div class="solution-actions"><button class="quiet-btn" id="back-to-recheck">← 재점검 리스트</button><button class="about-cta" id="reanalyze-project">새 분석 시작 →</button></div>`;
