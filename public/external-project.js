  ...
  const openRecheck = () => {
    ...
    document.querySelector("#run-recheck").addEventListener("click", async event => {
      ...
      try {
        ...
        const applied = result.applied.map((item, index) => `<article class="recheck-row"><b>${String(index + 1).padStart(2, "0")}</b><div><strong>${escape(item.file)} · ${escape(item.line)}번째 줄</strong><p>수정 전: ${escape(item.before)}</p><p>수정 후: ${escape(item.after)}</p></div><em>적용 완료</em></article>`).join("") || `<p>자동으로 적용할 수 있는 SHA 고정 항목이 없습니다.</p>`;
        solutionPage.innerHTML = `<p class="eyebrow">RECHECK COMPLETE</p><h2>Codex 수정 사본을<br/>다시 점검했습니다.</h2><p class="repair-intro">${escape(result.note)}</p><section class="repair-recheck"><span>정적 분석 비교</span><h3>${result.before.findings}건 → ${result.after.findings}건</h3><p>수정 가능한 GitHub Action 버전 고정 항목을 로컬 사본에 적용한 뒤, 동일한 Semgrep 규칙으로 다시 검사했습니다.</p></section><section class="recheck-list">${applied}</section><div class="solution-actions"><button class="quiet-btn" id="back-to-repairs">← 수정안 다시 보기</button><button class="about-cta" id="reanalyze-project">새 분석 시작 →</button></div>`;
        document.querySelector("#back-to-repairs").addEventListener("click", () => window.vibeNavigate?.("solution"));
