...
발표 자료 전체는 [PDF](docs/submission/codex-community-full.pdf)로, 채점에 필요한 장면만 모은 자료는 [발췌 PDF](docs/submission/codex-community-evaluation-extract.pdf)로 확인할 수 있습니다. 아래 설명은 발표 자료와 실제 저장소 구현을 함께 기준으로 작성했습니다. 개인 프롬프트와 API 키는 포함하지 않았습니다.

### 기술적 구현과 안정성

- 공개 GitHub 저장소를 **읽기 전용으로 복제**하고, 파일 구조·API·인증 관련 코드를 분석합니다.
- [Semgrep](https://semgrep.dev/)을 실제로 실행해 코드 근거를 찾고, 실행할 수 없으면 결과를 만들지 않고 `UNAVAILABLE`로 표시합니다.
- 승인된 로컬 테스트 대상에서는 실제 HTTP 요청 → 증거 수집 → 규칙 판정 → 패치 → 동일 요청 재실행을 확인합니다.
- `npm test`는 Golden Demo의 `HTTP 200 → 위반 → 승인 → 패치 → HTTP 403 → PASS` 흐름을 검증합니다.
- 발표 자료 발췌 PDF 4쪽은 저장소 clone부터 코드 구조 분석, 기능 단위 추상화, Semgrep 연결까지의 구현 구조를 보여줍니다.

### 제품 완성도와 사용자 경험

- 파일명·규칙 ID를 기본 화면에 나열하지 않고, **홈·로그인·프로필·외부 연동** 같은 서비스 기능을 노드로 보여줍니다.
- edge에는 로그인 정보 전달, 개인정보 반환, 외부 주소 요청처럼 정보 이동을 적습니다.
- 위험 가능성이 있는 edge만 빨간색으로 표시하고, 클릭 시 자연어 설명과 실제 코드 근거를 확인합니다.
- 발표 자료 발췌 PDF 5쪽은 정상 흐름과 위험 흐름을 같은 보안 지도에서 구분하는 방식을 보여줍니다.

### 문제와 아이디어의 명확성

바이브 코딩은 구현 속도를 높이지만, 만든 서비스의 데이터·권한 흐름을 이해하고 검증하는 과정까지 자동으로 보장하지는 않습니다. VibeCheck는 취약점 목록을 보여주는 대신 **서비스 구조 → 위험 흐름 → 근거 → 수정 검토 → 재점검**의 흐름을 제공합니다. 발표 자료 발췌 PDF 2쪽과 3쪽은 이 문제와 제품 방향을 설명합니다.

### 잠재적 영향력과 사용자 가치

- 보안 전문가가 아닌 개발자도 “어떤 기능에서 어떤 데이터가 어디로 가는지”부터 이해할 수 있습니다.
- 회사 보안 정책 문서를 업로드하면, 결과를 설명할 때 프로젝트의 허용·금지 기준을 함께 확인합니다.
- 공개 저장소 분석은 원격 코드를 바꾸지 않으며, 외부 서비스에 임의의 공격 요청을 보내지 않습니다.
- 승인된 테스트 환경에서만 실제 요청 검증을 진행합니다.

### 서로 다른 역할 간 협업

- **Build**: 서비스 전체 구조, 프론트엔드, 보안 지도·대시보드 UI, 사용자 흐름을 구현했습니다.
- **AI 모델**: Security Facts 추출과 신경망·규칙 기반 검증 파이프라인을 설계했습니다.
- **Insight**: 문제 정의, 경쟁·시장 관점, 사용자 경험, 발표 자료와 가치 제안을 정리했습니다.
- 발표 자료 발췌 PDF 7쪽에는 팀의 역할 분담을 정리했습니다.

### Codex를 계획·구현·검토·검증에 활용한 방식

1. **계획**: 저장소 분석, 보안 규칙, 서비스 흐름을 연결하는 구현 순서를 설계했습니다.
2. **구현**: clone·Semgrep·코드 사실 추출·보안 지도·재점검 화면을 구현했습니다.
3. **검토**: 초기 그래프가 코드 분석 순서처럼 보인다는 피드백을 반영해, 서비스 기능과 데이터 흐름 중심으로 재설계했습니다.
4. **오류 복구**: 라우팅, 재점검 버튼, Vercel 서버리스 배포 오류를 진단하고 수정했습니다.
5. **검증**: Golden Demo 테스트와 배포 응답을 확인했습니다.

완료된 작업과 아직 확장 단계인 기능은 [Codex Build Log](docs/CODEX_BUILD_LOG.md) 및 [JSON 로그](docs/codex-build-log.json)에 구분해 기록했습니다. Codex API 수정안은 키가 설정된 경우에만 호출하며, 한 파일만 바꾸는 unified diff인지 검사한 뒤 새 로컬 복제본에만 적용합니다.

### 행사 이후에도 발전할 수 있는 가능성

현재 웹 MVP의 분석 코어는 향후 CLI, IDE Plugin, GitHub Action에서 재사용하는 구조를 목표로 합니다.
...
다음 단계는 개발 중인 코드에 즉시 보안 지도를 제공하는 IDE Plugin, PR마다 분석을 실행하는 GitHub Action, 회사 정책과 내부 문서를 연결하는 Enterprise 흐름입니다. 발표 자료 발췌 PDF 6쪽에는 이 확장 계획을 정리했습니다.
